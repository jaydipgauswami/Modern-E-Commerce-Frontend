...
import { 
  FaBoxOpen, FaHeart, FaHeadset, FaTags, 
  FaUserCircle, FaMapMarkerAlt, FaCog, FaSignOutAlt,
  FaChevronRight, FaRegEdit, FaCamera
} from "react-icons/fa";
import { useCart } from "../../context/CartContext";

// --- Types ---
type Tab = "profile" | "orders" | "wishlist" | "addresses" | "settings" | "cart";




const MOCK_ORDERS = [
  { id: "#ORD-9821", date: "Oct 24, 2023", status: "Delivered", amount: "$129.99", items: 2 },
  { id: "#ORD-9754", date: "Sep 12, 2023", status: "Processing", amount: "$45.00", items: 1 },
  { id: "#ORD-9632", date: "Aug 05, 2023", status: "Shipped", amount: "$210.50", items: 4 },
];
            ...
            <nav className="flex lg:flex-col overflow-x-auto lg:overflow-visible gap-2 pb-4 lg:pb-0 scrollbar-hide">
              {NAV_ITEMS.map((item) => (
                <button
                  ...
                  `}
                >
                  <item.icon className={`text-lg mr-3 ${activeTab === item.id ? "text-indigo-200" : "text-gray-400 group-hover:text-indigo-500"}`} />
                  {item.label}
                  {activeTab === item.id && (
                    <motion.div 
                      layoutId="activeTab" 
              ...
              <Link href="/help" className="hidden lg:flex items-center px-4 py-3 text-sm font-medium text-gray-600 rounded-xl hover:bg-white hover:shadow-sm transition-all">
  ...
  Edit Profile
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-5 rounded-xl border border-gray-100 bg-gray-50/50">
          <h3 className="text-sm font-semibold text-gray-900 mb-4 uppercase tracking-wider">Personal Information</h3>
        ...
        <div className="p-5 rounded-xl border border-indigo-100 bg-indigo-50/30">
          <h3 className="text-sm font-semibold text-indigo-900 mb-2 uppercase tracking-wider">Store Credits</h3>
          <p className="text-3xl font-bold text-indigo-600">$120.00</p>
          <p className="text-xs text-indigo-500 mt-2">Available for your next purchase.</p>
          <Link href="/coupons" className="inline-flex items-center gap-2 mt-4 text-sm font-medium text-indigo-600 hover:text-indigo-800 group">
            <FaTags /> View Coupons 
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/orders" className="text-sm text-indigo-600 hover:text-indigo-800 font-medium hover:underline">View all</Link>
      </div>
      <div className="space-y-4">
        {MOCK_ORDERS.map((order, i) => (
          <div key={i} className="flex flex-col sm:flex-row items-center justify-between p-4 sm:p-5 rounded-xl border border-gray-100 hover:shadow-md transition-shadow bg-white">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Receive updates on orders and promotions.</p>
          </div>
        ...
        <div className="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Get text messages for delivery updates.</p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
